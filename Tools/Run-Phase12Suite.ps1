param(
    [string]$Method = 'DungeonRoguelite.Editor.Phase12VerificationRunner.Run',
    [string]$Suite = '12_1',
    [string]$Name = 'gate12_1',
    [string]$Extra = '',
...
$extraArgs = "-gateSuite $Suite $Extra".Trim()
$arguments = "-batchmode -nographics -projectPath `"$project`" -executeMethod $Method -logFile `"$log`" $extraArgs"
...
if ($editor.ExitCode -eq 0 -and $Extra -notmatch '-quit') {
    ...
    if ($content -notmatch '(?m)^\[(?:[^\r\n]*COMPLETE|M8\.4 TEST SUCCESS)\][^\r\n]*PASSED' -or
        $content -notmatch '\[PHASE 12 STATE RESTORED\]' -or
