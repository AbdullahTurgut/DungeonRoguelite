...
- Preview refresh runs immediately after the existing offered-weapon selection. Name/tier/stat formatting and selection/equip/claim/state logic remain unchanged. Missing sprites clear the previous image, disable the empty Image and show a restrained GORSEL YOK label instead of a white square.
- Nine 256px static sprites baked from the exact approved Hub weapon mappings: Base/Tier I/Tier II for Warrior, Archer and Gunner. Visually inspected all nine exported files. Editor-only ArmoryWeaponPreviewAuthoring can regenerate them; it copies mesh/material data into a temporary preview renderer and adds no runtime camera, model or render texture.
- Only existing scene blocks changed: equipment text RectTransform (space for slot), equipment panel child list, and ArmoryHubUI serialized preview bindings. Hero meshes/transforms/mappings, buttons and the rest of the scene are preserved. Gameplay, stats, saves, progression, enemies, runtime weapons and Phase 26A VFX files untouched. All earlier local edits except this handoff document retain their starting hashes.
- Validation: Unity compilation and icon authoring PASS (exit 0), three existing TMP obsolete warnings, no C# errors or exceptions in the successful run. Initial sandbox launch stalled on licensing; stopped it and completed outside sandbox. All nine sprite entries are bound and scene saved/reopened successfully. Evidence: Logs/Phase26A1/bake.log and before-hashes.json. No broad tests or gameplay automation.
- NEXT: user manual QA in Hub_Armory: open detail popup, cycle Base/Tier I/Tier II for each hero through the existing campaign flow, check icons/text/equip states and optionally clear one sprite to check fallback. No commit or push. Stop after this implementation; 26A combat visual QA also remains pending.

## Phase 26A - player combat VFX baseline (2026-09-15; manual QA pending)

This checkpoint supersedes the historical next-task instructions below. Approved/pushed baseline is 2b515c7 (Phase 25 Armory/runtime presentation), following approved enemy obstacle avoidance. Phase 26A is implemented locally; no commit, push, staging, save/progression changes, or later polish phases.

- Reused WarriorSlashFeedback.OnAttack subscription and the existing BowWeapon/RifleWeapon presentation calls. Old logic handled Tier I only; Tier II fell back to Base. Arrow.prefab had no TrailRenderer, and the slash/tracer borrowed materials that did not reliably support vertex tint/alpha.
- AttackVfxStyle reads the real PlayableCharacter definition and PermanentProgression.GetEquippedWeapon().Tier on attacks. One project-owned transparent, depth-tested, vertex-colored unlit shader/material is shared through Resources, ensuring build inclusion without per-shot material creation.
- Warrior: silver tapered crescent / thicker ember crescent / gold outer crescent plus white-hot inner accent. Existing radius fraction and 0.12-second visual lifetime retained, with fade and disable cleanup. Melee geometry and timing untouched.
- Archer: short minimal neutral trail / cyan 0.07-second trail / violet 0.095-second trail with compact cyan chevron. Visual-only body tint; no projectile-root scaling, collision edits, or bow/anchor edits.
- Gunner: warm neutral / cyan coil / violet premium tracer and compact muzzle flash. Maximum tracer widths 0.035 / 0.05 / 0.065 metres. Existing 0.05s tracer and 0.04s flash duration retained; presentation expires during pauses. Only the dedicated runtime flash mesh scale changes; MuzzlePoint, hitscan and weapon attachments are untouched.
- No new impact bursts: current melee/projectile hooks do not consistently expose confirmed impact position, and the rifle shot event includes misses. Defer a narrow confirmed-hit presentation hook to 26B; existing enemy hit flash is untouched.
- Validation: C# compile PASS with three existing obsolete TMP warnings. Initial editor loading caught a RifleWeapon MaterialPropertyBlock field-initializer exception; moved creation to the feedback method, requiring one corrective automatic recompile. Correction compiled successfully, followed by D1 opening with no new exceptions. Shader/material import succeeded. No verifier, harness, automated gameplay run, or broad suite was added/run.
- Quick GUID reference sanity PASS for all three hero prefabs, arrow, D1, and new material (including package references). Source comparison confirms unchanged MeleeWeapon, ArrowProjectile, Rifle ExecuteAttack, locked weapon values, and all scene/prefab/ScriptableObject/ProjectSettings files. Seven protected material/TMP files retain their starting SHA256 hashes; untouched existing whitespace remains.
- Evidence: Logs/Phase26A/editor.log, reference-sanity.txt and protected-before.json (ignored). Unity is open in edit mode on Assets/Scenes/Dungeons/Dungeon_Prototype.unity (Dungeon 1), confirmed by the load log and Library/LastSceneManagerSetup.txt. Startup scene argument was ignored; opened through Unity's Open Scene dialog without saving the scene.
- NEXT: user manual QA of Warrior/Archer/Gunner Base -> Tier I -> Tier II via normal character selection and Armory equipment flow, then Dungeon 1. No debug equipment overrides or save unlocks were introduced. Check tier distinction, top-down readability, rapid-fire clutter and unchanged mechanical feel. READY means ready for manual testing, not visually accepted; runtime appearance and 1080p/60 performance have not been manually verified. Stop here; no commit/push or 26B/26C work.



Cardinal weapon integration (2026-09-14): user authorized preserving the four unrelated D8/D10 material changes completely untouched. Their SHA256 hashes remain identical after the single Unity compile/authoring pass. Added real CardinalZebra Greatsword.fbx to warrior_tier_1 and Longbow.fbx to archer_tier_1, under the existing hand attachments; Base objects, Gunner bindings, Blacksmith/layout and runtime equip/save/combat code were preserved. Source folder: Assets/Art/ThirdParty/CardinalZebra_FantasyWeapons/Low Poly Fantasy Weapon Pack/. Longsword/Shortbow were inspected but not assigned as stronger Tier II upgrades. Added the source-palette URP material and concise provenance retaining credits.txt. The existing SetupHubArmory tool now has a targeted reusable Cardinal integration entry point; no new temporary Assets helper was created.

Compilation PASS (one pass, three existing TMP warnings); scene reopened and actual serialized Tier I IDs/mesh references confirmed. Hub_Armory is open. Posed preview Logs/Phase25/hub-cardinal-tierI.png reveals an attachment-orientation blocker: Archer Longbow is nearly edge-on, and Greatsword points outward rather than along the previous two-hand hold. Do not claim final visual acceptance. NEXT: correct only these new mesh-node rotations against each starter mesh's actual local axes, preserving their wrapper/hand anchors; the user limited this task to one authoring pass, so no second pass was performed. Manual click QA remains pending; Tier II slots still fall back to Base. No commit, push, staging or writes to the protected four materials.

Phase 25 binding-only audit (2026-09-14): user approved the current Hub composition enough to preserve. Inspected actual serialized bindings and runtime read/refresh flow. Warrior and Archer had empty tierWeapons arrays; Gunner had only gunner_tier_1. The saved equipped IDs observed by Unity were warrior_tier_2, archer_tier_1 and gunner_tier_2: all three lacked a visual mapping, so RefreshVisuals correctly selected Base while equipment text changed. UI equip/base actions already refresh the explicitly bound presentations synchronously; no callback defect was found. Added explicit catalog-ID slots with null references for unavailable visuals, preserving Gunner's valid Base blaster-g / Tier I blaster-p mapping. This exposes missing assets rather than claiming a fabricated swap. No meshes, composition, NPC, UI styling, gameplay or save data changed. The scene-only delta is 12 additions/2 removals in mapping arrays, authored by Unity APIs. One compilation passed with existing TMP warnings; reopened bindings and active meshes were inspected in Unity, and Hub_Armory is open for manual QA. Evidence: Logs/Phase25-bindings.log. No broad tests, commit or push. NEXT: provide/approve actual Warrior/Archer tier visuals and any desired Tier II visuals; complete visible three-hero swaps cannot be achieved through bindings alone with those references absent. Gunner Base/Tier I is bound; manual click QA remains pending.

Phase 25 recovery (2026-09-14; supersedes the stashed-WIP next action below): continued the uncommitted Hub WIP on approved HEAD 30679ea. No commit/push; stash@{0} untouched. The actual WIP bound equipment to obsolete primitive nodes rather than the visible hand-mounted weapons, used a rejected one-handed sword/crossbow for Tier I, and scaled the Armorer crate 100x. Repaired the existing scene through Unity APIs without rebuilding hero displays. Explicit ArmoryHubUI presentation references now refresh on equip/base/reward refresh; missing visual tiers retain the starter. Removed obsolete Hub weapon geometry/copies, including fake Tier II variants; no new Tier II art is claimed.

Current asset blockers: Warrior Base remains KayKit/sword_2handed.fbx; the downloaded sword_2handed_color has identical geometry, and the other sword/axes do not satisfy upgraded greatblade identity. Archer Base remains KayKit/bow.fbx; bow_withString is the same design with string fittings, not an accepted distinct Tier I bow. Both Tier I visuals deliberately fall back to Base pending suitable approved models. Gunner Hub uses Kenney blaster-g.fbx for Base and blaster-p.fbx for Tier I on the existing hand attachment. Posed camera renders confirm distinct models; actual KUŞAN/Base clicks remain manual QA, not a claimed runtime PASS.

Armorer: complete KayKit Barbarian, bear hat hidden, sampled Idle_A pose, grounded feet, a correctly scaled inspection sword and small storage crate; moved clear of hero silhouettes/header and placed interaction beside it. Final camera render includes UI; user visual approval remains pending. Ownership UI keeps claimed rewards off AL, shows KUŞAN only when owned/unequipped, green equipped status without a redundant action, and BAŞLANGIÇ. Tier I-only slash/arrow/tracer differentiation is implemented; removed projectile-root scaling/material allocation and preserved authored Base tracer appearance. Combat VFX readability still requires manual QA.

Validation: Unity 6000.3.23f1 compilation succeeded (three existing obsolete TMP warnings, no C# errors); initial sandbox licensing refusal resolved outside sandbox. Production-scene missing-script/reference inspection logged no errors. All three passive hero hierarchies contain only HubHeroPresentation as a MonoBehaviour. Original hero roots, ProjectileSpawnPoint and MuzzlePoint transforms compare unchanged against the pre-recovery WIP; gameplay prefabs, definitions, progression/save files and ProjectSettings have no diffs. Unity opened Hub_Armory interactively for QA. No lifecycle/regression suites or save mutations. Evidence and AssetDatabase dependency list are under ignored Logs/Phase25; the temporary editor authoring/inspection scripts remain until QA. Removed 17 root setup/batch/query logs. Full downloaded packs and licenses/provenance retained. Two pre-existing unrelated D10 material changes remain untouched; Unity scene serializer whitespace remains (no manual YAML cleanup).

NEXT: manual review of the recovered Hub/Gunner/Armorer/UI and Tier I VFX; obtain suitable distinct Warrior greatblade and Archer bow before declaring Phase 25 ready. Do not commit, push, pop the stash, or prune packs before user QA. Worktree remains dirty, approximately 966 changed/untracked files including pack imports and metadata.

Environment rework final sign-off (2026-09-13; supersedes the historical D2/D3 visual description below): USER MANUAL QA ALL GREEN for D2, D3, D4 and D5. Approved visuals frozen: D2 cold cellar with blind masonry bays; D3 sealed burial niches and processional paving; D4 ruined colonnades and long hall bands; D5 stepped ashen sanctuary with flush obelisk framing. Reused the existing CC0 KayKit Dungeon subset and retained its provenance/license records; all eleven new project-owned materials are referenced by the scenes. Temporary environment authoring/setup files are absent. Final verification: approved scene bytes unchanged during finalization; all 314 existing camera/script/physics components, protected hierarchy activation and authoritative transforms preserved (ignoring serializer trailing whitespace only), with no added gameplay components. Spawns, waves, XP, camera, progression, Ash Warden files and original obelisk objects remain unchanged. All 97 prefab source references resolve; previous four-scene reopen checks passed and final compile reports zero broken PrefabInstance errors. Final Unity 6000.3.23f1 batchmode compilation PASSED, exit 0, no C# errors or warnings; initial sandbox licensing failure was resolved by the approved external retry. Only the four scenes, eleven materials with metadata and this status file belong to the local environment checkpoint. No push. NEXT: STOP; do not restore/pop stash@{0}. Phase 25 Hub WIP remains safely stashed at ae1003a3e04f7a8a68b619af8e32befa445f58c6 until explicitly authorized.
